...
contract EIP712WhitelistQty is Adminable, Context {
    ...
    modifier checkWhitelist(bytes calldata _signature, uint256 _qty) {
        require(eip712Signer == _recoverSigner(_signature, _qty), "EIP712: Invalid Signature");
        _;
    }

    function verifySignature(bytes calldata _signature, uint256 _qty) public view returns (bool) {
        return eip712Signer == _recoverSigner(_signature, _qty);
    }

    // Recover the signer address
    function _recoverSigner(bytes calldata _signature, uint256 _qty) internal view returns (address) {
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(HASH_STRUCT, _msgSender(), _qty))
            )
        );
