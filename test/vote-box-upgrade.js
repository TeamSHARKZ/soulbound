...
describe('Vote Box - UUPS', function () {
  let contract;
  let ownerKey;
  let whitelistKey;
  let guestKey;
  ...
  beforeEach(async function () {
    ...
    // Deploy VoteBox v1
    const v1 = await ethers.getContractFactory('VoteBoxV1');
  ...
  it('v1 -> v2 -> v1', async function () {
    ...
    // upgrade to v2
    const v2Contract = await ethers.getContractFactory('VoteBoxV2');
    contract = await upgrades.upgradeProxy(contract.address, v2Contract, []);
    expect(await contract.version()).to.equal('2');

    // rewind to v1
    const v1Contract = await ethers.getContractFactory('VoteBoxV1');
